...
Basic Accuracy: 43.96%  Submissions: 145K+ Points: 1
...
that takes the head of link list and integer value of the data to be inserted as 
inputs and returns the head of the modified link list. 
...
public class LinkedListInsertion {
	...
	public static Node insertAtBeginning(Node head, int x) {
		Node node = new Node(x); // create node
		node.next = head; // Add head to the node next value
	...
	public static Node insertAtEnd(Node head, int x) {
		Node node = new Node(x); // create node
		if (head == null) { // Check null node
			return node; // Return new node as head
		}
		Node tmpHead = head; // Copy head in tmp object
		while (tmpHead.next != null) { // Move to the end of linkedList
			tmpHead = tmpHead.next; // Move to next node
		}
		tmpHead.next = node; // Add new node at the end
		return head; // Retrun head
	}
}